...
var Frame = {
    ...
    addMenuItems : function(element, menus, type) {
		...
		if(type == "main") {
            ...
            for (var i = 0, len = menus.length; i < len; i++) {
				...
				if("Device Information" == menus[i].name || "Infor. do dispositivo" == menus[i].name || "デバイス情報" == menus[i].name || "Información del dispositivo" == menus[i].name || ("设备信息" == menus[i].name))
				{
					posDeviceInfo = i;
				}
	...
	setCopyRightInfo : function(language) {
        if (language == "english") {
            ...
            $("#footerText").html("版权所有 © 华为技术有限公司 ##.COPY_RIGHT_YEAR.##。保留一切权利。");
        }
    },
